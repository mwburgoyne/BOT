...
def test_undersaturated_oil_is_monotone_when_enforced():
    table = read_excel(DATA, surface=SURFACE)
    cfg = Config()
    cfg.auto_apply_fixes = True
    cfg.convergence_pressure_Pk = 6000.0
    cfg.enforce_undersaturated_monotonic = True
    res = pipeline.build(table, cfg)
    # every undersaturated oil branch has non-increasing Bo (above bubble point)
    for rows in res.extended.pvto.usat:
        if rows.shape[0] >= 2:
            assert np.all(np.diff(rows[:, 1]) <= 1e-12)
    # and the low-pressure artifact no longer flags
    assert not [a for a in res.diagnostics.anomalies
                if a.kind == "negative_undersaturated_compressibility"]


def test_blind_spot_oil_branch_regenerated_physically():
    table = read_excel(DATA, surface=SURFACE)
    cfg = Config()
    cfg.auto_apply_fixes = True
    cfg.convergence_pressure_Pk = 6000.0
    res = pipeline.build(table, cfg)
    # the low-pressure node(s) were regenerated by the reciprocal-family law
    regen = [a for a in res.diagnostics.anomalies
             if a.kind == "undersaturated_regenerated"]
    assert regen
    # the psat=100 branch now has a gentle, physical compressibility (not 10x)
    e = res.extended
    ...
    assert 1e-7 < c_o < 1e-5            # physical range, not over-steep
...
def test_change_log_records_fixes_with_reasons():
    ...
    assert "Regenerated" in actions                          # blind-spot rebuild
